...
class Seksi extends CI_Controller
{
	...
	function blank()
	{
		$this->load->view('pkc/seksi/v_head');
		$this->load->view('pkc/seksi/v_blank');
		$this->load->view('pkc/seksi/v_foot');
	}
	...
	{
		$this->load->view('pkc/seksi/v_head');
		$this->load->view('pkc/seksi/v_index');
		$this->load->view('pkc/seksi/v_foot');
	}
	...
	function list_jaminan()
	{
		...
		$this->load->view('pkc/seksi/v_head');
		$this->load->view('pkc/seksi/v_list', $data);
		$this->load->view('pkc/seksi/v_foot');
	}
	...
	function monitoring()
	{
		...
		$this->load->view('pkc/seksi/v_head');
		$this->load->view('pkc/seksi/v_monitoring', $data);
		$this->load->view('pkc/seksi/v_foot');
	}
	...
	function detail($id)
	{
		...
		$data['data_izin'] =
			$this->db->query(
				"SELECT * FROM data_izin as A 
						...
						JOIN bchanggar as G ON G.id_hanggar = B.id_hanggar
						JOIN bcuser as H ON H.id_bc = G.id_bc
						WHERE A.id_surat = $id
						ORDER BY A.id_surat DESC"
			)->result();
		...
		$this->load->view('pkc/seksi/v_head');
		$this->load->view('pkc/seksi/v_detail', $data);
		$this->load->view('pkc/seksi/v_foot');
	}
	...
	function detailmon($id)
	{
		...
		//Halaman Detail Monitoring -->Data Revisi
		$data['keluar'] =
			$this->db->query("SELECT * FROM data_keluar
			WHERE id_surat = $id AND jns_dok ='1'")->result();

		$data['masuk'] =
			$this->db->query("SELECT * FROM data_masuk
			WHERE id_surat = $id AND jns_dok ='2'")->result();

		$this->load->view('pkc/seksi/v_head');
		$this->load->view('pkc/seksi/v_detailmon', $data);
		$this->load->view('pkc/seksi/v_foot');
	}
