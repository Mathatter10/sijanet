<?php			
			...
			<?php foreach($jaminan as $j): ?>
				<?php
				$kb = preg_replace('/\s+/', '', $j->nama_tpb);
				$angka = preg_replace('/\D/', '', $j->no_jaminan);
				$qr = $angka.$kb; 
				$a = site_url('createpdf/QRcode/'.$qr); ?>
			<head>
			...
			<table border="0">
				...
				<tr>
					<td width="50">
						<br/>
						<img src="<?php echo $a ?>" width="40" height="40"/>
					</td>
					<td width="450">
						<p>Formulir ini dicetak secara otomatis oleh sistem komputer dan tidak memerlukan tanda tangan dan cap perusahaan </p>
					</td>					
				</tr>
			</table>
